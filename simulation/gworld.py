...
class GridWorld:
    ...
    def change_goal(self, agent_id, goal):
        self.aindx_goal[agent_id] = goal

    def path_to_action(self, aindx, path):
        actions = []
        print('Path: ', path)
        print('Length Path: ', len(path))
        cy, cx = path[0]# self.aindx_cpos[aindx]
        for step in path[1:]:
            ty, tx = step[0], step[1]
            if (tx - cx == 1):
    ...
    def parallel_step(self, aindx, action):

        y, x = self.aindx_cpos[aindx]
        self.aindx_cpos_prev[aindx] = (y, x)
        self.cells[y][x] = 0
    ...
    def check_collision(self):
        # Check collision after step of all agents to simulate parallel steps
        for index1 in self.aindx_cpos.keys():
            for index2 in self.aindx_cpos.keys():
                if not index1 == index2:
                    if self.aindx_cpos[index1] == self.aindx_cpos[index2]:
                        return [index1, index2], True
                    else:
                        return [], False

        for index1 in self.aindx_cpos.keys():
            for index2 in self.aindx_cpos_prev.keys():
                if not index1 == index2:
                    if self.aindx_cpos[index1] == self.aindx_cpos_prev[index2]:
                        if self.aindx_cpos[index2] == self.aindx_cpos_prev[index1]:
                            print('Swap places')
                            while 1:
                                pass
    
    def get_pos(self, index):
        return self.aindx_cpos[index]
    ...
    def get_agents(self):
        return self.aindx_cpos
    def get_goals(self):
        return self.aindx_goal
