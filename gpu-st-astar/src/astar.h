...
cpuAStar(const Graph &graph, std::vector<Position> sourceVec, std::vector<Position> destVec);
...
std::vector<std::vector<Node>>
gpuGAStar(const Graph &graph, std::vector<Position> sourceVec, std::vector<Position> destVec,
          const boost::compute::device &clDevice = boost::compute::system::default_device());
